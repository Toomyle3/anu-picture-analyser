...
const MusicGeneration = () => {
  ...
  const createAudio = useMutation(api.audios.createAudio);
  const router = useRouter();
  const [music, setMusic] = useState<string>();
  const handleGenerateMusic = useAction(api.music.generateMusicAction);
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      setMusic('');
      setIsLoading(true);
      const response = await handleGenerateMusic({
  ...
  const handleSubmitAudio = async () => {
    try {
      ...
      await createAudio({
        audioDescription: form?.getValues("promptMusic"),
        audioUrl: music!.toString(),
        views: 0,
      });
  ...
  return (
    <section>
      ...
      <div className="pt-[1rem]">
        {isLoading && <Loader content="Tommy is crafting a piece..." />}
        {!music && !isLoading && <EmptyState title="No Music generated." />}
        {music && (
          <Card
          ...
          >
            ...
            <CardFooter className="flex justify-center flex-col sm:flex-row gap-2">
              ...
              sm:w-[150px]
               ...
               transition-all duration-500 hover:bg-black-1"
              >
                {isSubmitting ? "Publicising" : "Public Music"}
              </Button>
            </CardFooter>
          </Card>
        )}
